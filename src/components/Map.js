...
export default function Map() {
    ...
    try {
      ...
      if (data?.properties?.parameter?.ALLSKY_SFC_SW_DWN?.ANN) {
        ...
        if (typeof annualIrradiance === 'number' && !isNaN(annualIrradiance)) {
          ...
          return {
            maxSunshineHoursPerYear: Number(sunshineHours),
            carbonOffsetFactorKgPerMwh: Number(co2Savings),
          };
        }
      }
      return null;
  ...
  class roofPanel {
    ...
    constructor(points, index, solarData) {
      ...
      this.updateColor();
      this.area =
        window.google.maps.geometry.spherical.computeArea(points) * 10.7639;
      this.index = index;
      this.points = points;
    ...
    drawSolarPanelsInTriangle(points) {
      ...
      let [northMost, southMost] =
        maxLine[0].lat > maxLine[1].lat
          ? [maxLine[0], maxLine[1]]
          : [maxLine[1], maxLine[0]];

      let maxLineHeading = window.google.maps.geometry.spherical.computeHeading(
        northMost,
        southMost
      );
      ...
      while (canDrawPanels) {
        ...
        let nextRowPoint = window.google.maps.geometry.spherical.computeOffset(
          currPoint,
          2 * solarPanelHeight * 0.3048,
          180
        );
    ...
    drawRowOfPanels(origin, direction) {
      switch (direction) {
        case CardinalDirection.west:
          let topLeftPointWest =
            window.google.maps.geometry.spherical.computeOffset(
              origin,
              solarPanelWidth * 0.3048,
              270
            );
          let bottomRightPointWest =
            window.google.maps.geometry.spherical.computeOffset(
              origin,
              solarPanelHeight * 0.3048,
              180
            );
          let bottomLeftPointWest =
            window.google.maps.geometry.spherical.computeOffset(
              bottomRightPointWest,
              solarPanelWidth * 0.3048,
              270
            );
          let solarPanelVerteciesWest = [
            origin,
            topLeftPointWest,
            bottomLeftPointWest,
            bottomRightPointWest,
          ...
          if (fullyWithinRoofPanelWest) {
            ...
            this.solarPanels[this.solarPanels.length - 1].setMap(
              mapRef.current
            );
          }
        ...
        case CardinalDirection.east:
          let topRightPointEast =
            window.google.maps.geometry.spherical.computeOffset(
              origin,
              solarPanelWidth * 0.3048,
              90
            );
          let bottomRightPointEast =
            window.google.maps.geometry.spherical.computeOffset(
              origin,
              solarPanelHeight * 0.3048,
              180
            );
          let bottomLeftPointEast =
            window.google.maps.geometry.spherical.computeOffset(
              bottomRightPointEast,
              solarPanelWidth * 0.3048,
              90
            );
          let solarPanelVerteciesEast = [
            origin,
            topRightPointEast,
            bottomRightPointEast,
            bottomLeftPointEast,
    ...
    updateColor() {
      if (
        this.solarData &&
        this.solarData.maxSunshineHoursPerYear !== undefined
      ) {
    ...
    addBack() {
      ...
      setDeletedPanels((deletedPanels) =>
        deletedPanels.filter((item) => item !== this.index)
      );
    }
    ...
    updatePoints(newPoints, finalize = false) {
      ...
      if (finalize) {
        this.area =
          window.google.maps.geometry.spherical.computeArea(newPoints) *
          10.7639;
        this.solarPanels.forEach((panel) => panel.setMap(null));
        this.solarPanels = [];
          ...
          >
            <Box
              sx={{
                ...
                borderBottom: '50px solid #073845', 
                display: 'flex',
                alignItems: 'center',
