...
export default function Map({ currentPage }) {
    ...
    const [overlappingPoints, setOverlappingPoints] = useState({}); // Now stores { "panelIdx-pointIdx": { overlappedPanelIdx, overlappedPointIdx } }
    ...
    const getSelectedAreaCenter = () => {
        ...
        return { lat: latSum / boxPoints.length, lng: lngSum / boxPoints.length };
    };
    ...
    const latLngLiteral = { lat: 51.1657, lng: 10.4515 };

    const plusCursorSvg = `
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
    <!-- White outline for contrast and cool effect -->
    <path d="M12 4v16M4 12h16" stroke="white" stroke-width="3" fill="none" stroke-linecap="round" opacity="0.8"/>
    <!-- Bold black plus sign -->
    <path d="M12 4v16M4 12h16" stroke="black" stroke-width="1.5" fill="none" stroke-linecap="round"/>
    <!-- Subtle shadow effect -->
    <path d="M12 4v16M4 12h16" stroke="gray" stroke-width="3" fill="none" stroke-linecap="round" opacity="0.3" transform="translate(0.5, 0.5)"/>
  </svg>
`;
    ...
    const getGeocodingTypes = async (latLng) => {
        return new Promise((resolve) => {
            ...
            geocoder.geocode({ location: latLng }, (results, status) => {
                if (status === "OK" && results[0]) resolve(results[0].types);
                else resolve(["Geocoding Failed"]);
            });
        });
    };
        ...
        return new Promise((resolve) => {
            ...
            geocoder.geocode({ location: latLng }, (results, status) => {
                if (status === "OK" && results[0]) {
                    ...
                    const roofTypes = [
                        "premise", "building", "roof", "street_address", "sublocality",
                        "locality", "neighborhood", "postal_code", "store", "car_repair",
                    ];
                    const openAreaTypes = [
                        "park", "natural_feature", "forest", "grassland", "field",
                        "meadow", "tourist_attraction", "route",
                    ];

                    if (types.length === 2 && types.includes("establishment") && types.includes("point_of_interest")) {
                        resolve("Roof");
                        ...
                        if (roofTypes.some((type) => additionalTypes.includes(type))) resolve("Roof");
                        else if (openAreaTypes.some((type) => additionalTypes.includes(type))) resolve("Open Area");
                        else resolve("Roof");
                    } else {
                        if (roofTypes.some((type) => types.includes(type))) resolve("Roof");
                        else if (openAreaTypes.some((type) => types.includes(type))) resolve("Open Area");
                        else {
                            const components = results[0].address_components;
                            const hasInfrastructure = components.some((comp) =>
                                ["street_number", "route", "premise", "building"].includes(comp.types[0])
                            );
                            resolve(hasInfrastructure ? "Roof" : "Open Area");
                        }
                    }
                } else {
                    resolve("Roof");
    ...
    let addBoxPoint = async (coordinates) => {
        ...
        if (currentPage === "config" && roofPanels.length > 0 && !canDrawPanels) return;
        if (!canDrawPanels) return;

        const locationType = await checkLocationType(coordinates);
        console.log("Geocoding Types for Clicked Location:", await getGeocodingTypes(coordinates));
        ...
        if (boxPoints.length === 0) setBoxPoints([coordinates]);
        else setBoxPoints([...boxPoints, coordinates]);
    };

    useEffect(() => {
        if (boxPoints.length >= 2) {
            if (currPolyline.current !== undefined) currPolyline.current.setMap(null);
        ...
        } else if (currPolyline.current !== undefined) {
    ...
    class roofPanel {
        ...
        drawSolarPanelsInTriangle(points) {
            ...
            while (canDrawPanels) {
                ...
                if (nextRowPoint.lat < southMost.lat) canDrawPanels = false;
        ...
        drawRowOfPanels(origin, direction) {
            switch (direction) {
                case CardinalDirection.west:
                    ...
                    solarPanelVerteciesWest.forEach((vertex) => {
                    ...
                    if (fullyWithinRoofPanelWest) {
                        ...
                        newSolarPanel.addListener("click", () => this.delete());
                ...
                case CardinalDirection.east:
                    ...
                    let bottomLeftPointEast = window.google.maps.geometry.spherical.computeOffset(bottomRightPointEast, solarPanelWidth * 0.3048, 270);
        ...
        getFillColor(sunshineHours) {
            if (sunshineHours < 500) return "#FFFFFF";
            else if (sunshineHours < 1000) return "#FFD700";
            else if (sunshineHours < 1500) return "#FFA500";
            else if (sunshineHours < 2000) return "#FF0000";
            else return "#8B0000";
        }
        ...
        updateColor() {
            if (this.solarData && this.solarData.maxSunshineHoursPerYear !== undefined) {
                ...
                this.panel.setOptions({ fillColor: color, strokeColor: color });
        ...
        displaySideLengths() {
            if (this.sideLabels) this.sideLabels.forEach((label) => label.setMap(null));
            this.sideLabels = [];
    ...
    let getRoofArea = () => {
        ...
        roofPanels.forEach((panel) => {
            if (!panel.isDeleted) area += panel.area;
        });
        return area;
    };
    ...
    const downloadPDF = (panelIndex) => {
        ...
        if (input && roofPanels.length > panelIndex) {
            ...
            panel.points.forEach((point, index) => {
                ...
                if (index === 0) sketchCtx.moveTo(canvasX, canvasY);
                else sketchCtx.lineTo(canvasX, canvasY);
            });
            sketchCtx.closePath();
    ...
    const resetMapState = () => {
        ...
        if (currPolyline.current) currPolyline.current.setMap(null);
                ...
                >
                    {boxPoints.map((coordinates, index) => (
                        <Marker
                            ...
                            draggable={false}
                            onClick={() => {
                                if (index === 0 && boxPoints.length >= 3) addRoofSegmment(boxPoints);
                            }}
                        />
                    ))}
